...
{
	uintptr_t font_address = (uintptr_t)font;
	while (!mb_push_evt(EVT_SETFONT | (uint16_t)font_address, false));
	return font;
}
...
{
	while (!mb_push_evt(EVT_SETALIGNMENT | (uint16_t) alignment , false)) ;
	return alignment;
}

uint32_t lcd_set_direction(DC *dc, uint32_t direction)
{
	while (!mb_push_evt(EVT_SETDIRECTION | (uint16_t) direction , false)) ;
	return direction;
}
...
void lcd_draw_line(int16_t x1, int16_t y1, int16_t x2, int16_t y2, Color c)
{
	...
	while (!mb_push_evt(EVT_DRAWLINE | (uint16_t) c, false)) ;
	while (!handshake()) {}
}
...
void lcd_draw_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, Color c)
{
	...
	shdata[1].x = w;
	shdata[1].y = h;
	while (!mb_push_evt(EVT_DRAWRECT | (uint16_t) c , false)) ;
	while (!handshake()) {}
}
...
{
	if (n<MAXPOINTS) {
		while (!mb_push_evt(EVT_DRAWSEGMENTS|((uint16_t)n&0x7FF),false)) ;
		while (!handshake()) {}
	}
}
...
void lcd_draw_path2d(Graph *g, SPoint *p, int n)
{
	...
	for (int i = 1; i < n; ++i){
		...
		lcd_draw_line(startX, startY, endX, endY, g->color);
		startX = endX;
		startY = endY;
	}
}
...
void lcd_fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
	...
	while (!mb_push_evt(EVT_FILLRECT|((uint16_t)color), false));
	while (!handshake()) {}
}
...
void lcd_clip(int x1, int y1, int x2, int y2)
{
	...
	while (!mb_push_evt(EVT_CLIP, false));
	while (!handshake()) {}
}
...
void lcd_draw_string(DC *dc, int16_t x, int16_t y, const char *s)
{
	int length=strlen(s);
		...
		for(int i=0; i<length; i+=2){
			shdata[dataIndex].x = s[i];
			shdata[dataIndex].y = (i+1<length) ? s[i+1] : 0;
			dataIndex++;
		}
		while (!mb_push_evt(EVT_DRAWSTRING|(uint32_t)length, false));
		while (!handshake()) {}
}
