...
{
	if (s_Pll0_Freq==0) {
		uint32_t m=((SYSCON->PLL0SSCG1>>10) & 0xFFFF);
		uint32_t n=(SYSCON->PLL1NDEC & 0xFF) ? (SYSCON->PLL1NDEC & 0xFF) : 1;
		uint32_t p=(SYSCON->PLL1PDEC & 0x1F) ? 2*(SYSCON->PLL1PDEC & 0x1F) : 1;
		s_Pll0_Freq=s_Ext_Clk_Freq/n*m/p;
	}
    return s_Pll0_Freq;
}
...
{
	if (s_Pll1_Freq==0) {
		uint32_t m=(SYSCON->PLL1MDEC & 0xFFFF);
		uint32_t n=(SYSCON->PLL1NDEC & 0xFF) ? (SYSCON->PLL1NDEC & 0xFF) : 1;
		uint32_t p=(SYSCON->PLL1PDEC & 0x1F) ? 2*(SYSCON->PLL1PDEC & 0x1F) : 1;
		s_Pll1_Freq=s_Ext_Clk_Freq/n*m/p;
	}
    return s_Pll1_Freq;
}
